...
clear; close all; clc;
%%Competition model and it's stability
syms x y real
r_1_val=5;
r_2_val=2;
r_1 = r_1_val;
...
k_comp1 = 100;
k_comp2 = 100;
comp1 = r_1*x*(1-(x+a_12*y)/k_comp1);
comp2 = r_2*y*(1-(y+a_21*y)/k_comp2);
M = [comp1;comp2];
J = jacobian(M, [x,y]);
disp('Jacobian matrix is:');
disp(J);
[x_eq,y_eq] = solve([comp1==0, comp2==0], [x,y]);
...
tspan = [0 50];
initial_cond = [50, 50];
f = @(t, X) [r_1*X(1)*(1-(X(1)+a_12*X(2))/k_comp1);
             r_2*X(2)*(1-(X(2)+a_21*X(1))/k_comp2);];
[t, sol] = ode45(f, tspan, initial_cond);

figure;
plot(sol(:,1), sol(:,2), 'b', 'LineWidth', 2);
xlabel('Competitor 1 (x)');
ylabel('Competitor 2 (y)');
title('Phase Portrait');
grid on;


[x_q, y_q] = meshgrid(0:1:110, 0:1:110);
u = r_1.*x_q.*(1-(x_q+a_12.*y_q)/k_comp1);
v = r_2.*y_q.*(1-(y_q+a_21.*y_q)/k_comp2);              
figure;
hold on;
...
xlabel('Time');
ylabel('Population');
legend('Competitor 1', 'Competitor 2');
